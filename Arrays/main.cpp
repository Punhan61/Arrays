...
void main()
{
	...
	for (int i = 0; i < n; i++)
	{
		if (arr[i] < min)min = arr[i];// если в массиве значение элемента меньше нуля, то это условие не отработает.
		                              // чтобы этого избежать, проинициализировать переменные min и max нулевым элементом массива arr[0]
